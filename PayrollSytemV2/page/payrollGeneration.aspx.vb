...
Public Class payroll
    ...
    Protected Sub btnGenerate_Click(sender As Object, e As EventArgs) Handles btnGenerate.Click
        ...
        '  For x = 0 To gvGenerate.Rows.Count - 1
        Dim GetPayroll() As String = Nothing
        GetPayroll = DTGetPayroll(txtFrom.Text, TxtTo.Text)
        ...
        If gov_deduction_settings = "End Month" Then
            ...
            If w_hdmf = "1" Then
                emp_GD_PhilHealth_Amt = ComputeGDeduction(empCode, basicsalary, "PhilHealth")
            End If
        ...
        'empAbsent = ModC(ComputeAbsent(GetPayroll(0)) * GetPayroll(11))
        empTaxEarning = ModC(ModC(empBasicPay) + ModC(empOvertime)) - ModC(ModC(Late) + ModC(Undertime) + ModC(empGovDeduc))
        ...
        dt.Columns.Add(New DataColumn("Basic Pay", System.Type.GetType("System.String")))
    ...
    End Sub
    Friend Function DTGetPayroll(ByVal xFrom As DateTime, ByVal xTo As DateTime) As String()
        ...
        xSQL.AppendLine("WHERE ( `dtrcompute`.`dtrc_date` between @xFrom and @xTo ) ")
        Try
            Using SQLConnect As New MySqlConnection(My.Settings.DBConn)
                SQLConnect.Open()
                Dim SQLCommand As New MySqlCommand(xSQL.ToString, SQLConnect)
